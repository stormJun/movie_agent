...
class RetrievalExecutor(BaseExecutor):
    ...
    def __init__(
        ...
        *,
        ...
        prefer_retrieve_only: bool = False,
    ) -> None:
        ...
        self._prefer_retrieve_only = bool(prefer_retrieve_only)
    ...
    def _invoke_tool(self, tool: Any, task_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        默认优先使用structured_search，其次使用search，特殊工具（如chain_exploration）调用自定义方法。
        当 prefer_retrieve_only=True 时，会优先调用工具的 retrieve_only() 以避免工具侧生成最终答案。
        """
        if self._prefer_retrieve_only and hasattr(tool, "retrieve_only"):
            result = tool.retrieve_only(payload)
            if isinstance(result, dict):
                return self._ensure_retrieve_only_payload(result)
            return {"answer": str(result), "retrieval_results": []}
    ...
    @staticmethod
    def _ensure_retrieve_only_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        确保 retrieve_only() 输出在 multi_agent 流程中可被后续步骤消费：
        - 至少包含可读的 answer 文本（用于 reflection / debug）
        - 保留 retrieval_results 等证据字段
        """
        existing = payload.get("answer")
        ...
        if isinstance(context, str) and context.strip():
            answer_parts.append(context.strip())

        low_level = payload.get("low_level_content")
        if isinstance(low_level, str) and low_level.strip():
            answer_parts.append(low_level.strip())

        high_level = payload.get("high_level_content")
        if isinstance(high_level, str) and high_level.strip():
            answer_parts.append(high_level.strip())
