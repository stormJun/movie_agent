...
v3（Router-Worker 架构）下，本模块的定位发生了变化：

- Agent 只负责“检索证据”，对外仅暴露 `retrieve_with_trace(query, thread_id)`。
- “答案生成 / SSE 流式输出 / 会话状态管理”全部上移到服务侧（server/application/infrastructure）。
- legacy 的 `ask/ask_stream` + LangGraph checkpointer 已下线（仅保留 NotImplementedError 提示，避免误用）。

## 目录结构

```
backend/graphrag_agent/agents/
├── __init__.py                # 模块初始化文件
...
本模块在 v3 下不再在 Agent 内部构建/执行 LangGraph 工作流图；Agent 只做检索与证据汇总。

### 基类设计 (BaseAgent)

`BaseAgent` 类提供了所有 Agent 共享的基础功能：

1. **缓存管理**：保留缓存工具（用于检索侧/兼容旧数据），但不再负责“生成答案缓存”
2. **检索接口**：约定 `retrieve_with_trace()` 的返回结构（context / retrieval_results / reference）
3. **性能监控**：保留基础性能指标收集能力（可用于检索阶段）

```python
agent = GraphAgent(kb_prefix="movie")
trace = agent.retrieve_with_trace("推荐几部科幻电影", thread_id="session-123")

# trace = {
#   "context": "...",                 # 供服务侧统一拼接 prompt
#   "retrieval_results": [...],       # 结构化证据（可用于聚合/排序/去重）
...
4. **DeepResearchAgent**：v3 下仅保留“检索策略封装”（不在 Agent 内进行多轮思考/生成）
...
v3 strict 下，Agent 层不再提供 `ask_stream()`；流式输出统一由服务侧 SSE 接口实现（`/api/v1/chat/stream`）。
...
class FusionGraphRAGAgent(BaseAgent):
    ...
    def __init__(self):
        # v3 strict: Agents are retrieval-only and do not manage caches.
        super().__init__()
