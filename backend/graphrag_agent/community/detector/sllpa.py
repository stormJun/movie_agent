...
class SLLPADetector(GraphProjectionMixin, BaseCommunityDetector):
        ...
        try:
            ...
            CALL {
                ...
                CALL {
                    ...
                    MERGE (c:`__Community__` {id: $community_prefix + '0-'+toString(e.communityIds[index])})
    ...
    def _save_communities_fallback(self) -> Dict[str, int]:
        ...
        try:
            ...
            MERGE (c:`__Community__` {id: $community_prefix + '0-' + toString(primary_community)})
            ON CREATE SET c.level = 0, c.algorithm = 'SLLPA'
            MERGE (e)-[:IN_COMMUNITY]->(c)
            RETURN count(*) as count
            """, params={"entity_prefix": GRAPH_ENTITY_ID_PREFIX_FILTER, "community_prefix": GRAPH_COMMUNITY_ID_PREFIX})
