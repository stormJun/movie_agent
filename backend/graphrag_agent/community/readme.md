...
```
backend/graphrag_agent/community/
...
或在 `backend/config/rag.py` 中设置（不推荐，推荐用 `.env` 覆盖）：
```python
...
from graphrag_agent.ports.neo4jdb import get_db_manager, get_neo4j_config
from graphrag_agent.community import CommunityDetectorFactory

# 初始化图连接（确保已在启动时完成 provider 注入）
...
社区检测/摘要通常由运行层的构建流程统一触发；核心层只提供算法能力。
本仓库的默认构建流程见 `backend/infrastructure/readme.md`。
