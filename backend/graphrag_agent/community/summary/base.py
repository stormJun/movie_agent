...
class BaseCommunityRanker:
    ...
    def calculate_ranks(self) -> None:
        ...
        try:
            ...
            MATCH (c:`__Community__`)<-[:IN_COMMUNITY*]-(e:`__Entity__`)<-[:MENTIONS]-(d:`__Chunk__`)
            WHERE ($community_prefix = '' OR c.id STARTS WITH $community_prefix)
              AND ($entity_prefix = '' OR e.id STARTS WITH $entity_prefix)
              AND ($chunk_prefix = '' OR d.id STARTS WITH $chunk_prefix)
        ...
        for i in range(0, len(summaries), batch_size):
            ...
            try:
                ...
                SET c.summary = row.summary, 
                    ...
                    c.summary_error = row.summary_error
                """, params={"data": batch})
...
class BaseSummarizer(ABC):
    ...
    def __init__(self, graph: GraphClient):
        ...
        # 共库隔离：只处理指定前缀的社区/实体/Chunk（如 movie: / edu:）
        self.entity_id_prefix_filter = GRAPH_ENTITY_ID_PREFIX_FILTER or ""
        self.chunk_id_prefix_filter = GRAPH_CHUNK_ID_PREFIX_FILTER or ""
        self.community_id_prefix_filter = GRAPH_COMMUNITY_ID_PREFIX or ""

        # 控制社区摘要规模（避免 prompt 过大）
        self.max_nodes_per_community = COMMUNITY_SUMMARY_MAX_NODES
        self.max_rels_per_community = COMMUNITY_SUMMARY_MAX_RELS
        ...
        # 摘要并发独立于全局 MAX_WORKERS，便于按 LLM 限流调参
        self.max_workers = COMMUNITY_SUMMARY_MAX_WORKERS or MAX_WORKERS
    ...
    def _process_single_community(self, community: Dict) -> Dict:
        ...
        try:
            ...
            return {
                "community": community_id,
                "summary": "社区摘要生成失败（已存储 full_content 供检索）。",
                "full_content": stringify_info or str(community),
