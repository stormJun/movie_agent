...
class ChunkIndexManager(BaseIndexer):
    ...
    def clear_existing_index(self) -> None:
        """清除已存在的普通索引"""
        drop_index(self.graph, "chunk_embedding")
        drop_index(self.graph, CHUNK_VECTOR_INDEX_NAME)

    def create_chunk_index(self,
                         node_label: str = '__Chunk__',
                         text_property: str = 'text',
                         embedding_property: str = 'embedding',
                         index_name: str = CHUNK_VECTOR_INDEX_NAME,
                         id_prefix_filter: str = GRAPH_CHUNK_ID_PREFIX_FILTER) -> Optional[Any]:
        """
        ...
        # 同库多 KB：为当前 prefix 下的 chunk 节点打 KB label，用于索引隔离
        kb_label = (GRAPH_CHUNK_KB_LABEL or "").strip()
        ...
        # 获取所有需要处理的文本块节点
        where_parts = [
            f"c.{text_property} IS NOT NULL",
            f"c.{embedding_property} IS NULL",
        ]
        params: Dict[str, Any] = {}
        if id_prefix_filter:
            where_parts.append("c.id STARTS WITH $id_prefix")
            params["id_prefix"] = id_prefix_filter

        chunks = self.graph.query(
            f"""
            MATCH (c:`{effective_label}`)
            ...
            try:
                ...
                try:
                    ...
                    self.graph.query(
                        ...
                        FOR (c:`{effective_label}`) ON (c.`{embedding_property}`)
                        OPTIONS {{indexConfig: {{
