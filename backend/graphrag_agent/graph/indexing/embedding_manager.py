...
class EmbeddingManager:
    ...
    @staticmethod
    def _with_kb_prefix(value: str, kb_prefix: str) -> str:
        prefix = (kb_prefix or "").strip()
        if not prefix:
            return value
        full_prefix = f"{prefix}:"
        return value if value.startswith(full_prefix) else f"{full_prefix}{value}"

    def mark_document_chunks_for_update(self, filename: str, kb_prefix: str = "") -> int:
        """
        标记文档的所有Chunk需要更新Embedding

        Args:
            filename: 文件名（通常为原始文件名，不含 kb 前缀）
            kb_prefix: 知识库命名空间前缀（用于同库隔离，比如 "edu"；为空则不加前缀）

        Returns:
            int: 标记的Chunk数量
        """
        document_file_name = self._with_kb_prefix(filename, kb_prefix)
        query = """
        ...
        result = self.graph.query(query, params={"filename": document_file_name})
        marked = result[0]["marked"] if result else 0
        
        self.console.print(
            f"[blue]已标记文件 {document_file_name} 的 {marked} 个Chunk需要更新Embedding[/blue]"
        )
        
        return marked
    
    def mark_changed_files_chunks(self, changed_files: List[str], kb_prefix: str = "") -> int:
        """
        ...
        for filename in changed_files:
            ...
            marked = self.mark_document_chunks_for_update(file_name, kb_prefix=kb_prefix)
            total_marked += marked
