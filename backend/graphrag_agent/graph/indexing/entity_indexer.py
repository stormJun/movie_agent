...
from graphrag_agent.graph.core.graph_ops import (
...
class EntityIndexManager(BaseIndexer):
    ...
    def clear_existing_index(self) -> None:
        ...
        drop_index(self.graph, "entity_embedding")
        drop_index(self.graph, ENTITY_VECTOR_INDEX_NAME)

    def create_entity_index(self, 
                          node_label: str = '__Entity__',
                          text_properties: List[str] = ['id', 'description'],
                          embedding_property: str = 'embedding',
                          index_name: str = ENTITY_VECTOR_INDEX_NAME,
                          id_prefix_filter: str = GRAPH_ENTITY_ID_PREFIX_FILTER) -> Optional[Any]:
        """
        创建实体的向量索引，带批处理和并行优化
        ...
        # 同库多 KB：为当前 prefix 下的节点打 KB label，用于索引隔离
        kb_label = (GRAPH_ENTITY_KB_LABEL or "").strip()
        if kb_label and id_prefix_filter:
            self.graph.query(
                f"""
                MATCH (e:`__Entity__`)
                ...
                SET e:`{kb_label}`
                """,
                params={"prefix": id_prefix_filter},
            )

        effective_label = kb_label or node_label
        
        # 获取所有实体节点以准备批处理
        where_parts = [f"e.{embedding_property} IS NULL"]
        params: Dict[str, Any] = {}
        if id_prefix_filter:
            where_parts.append("e.id STARTS WITH $id_prefix")
            params["id_prefix"] = id_prefix_filter

        entities = self.graph.query(
            f"""
            MATCH (e:`{effective_label}`)
        ...
        if not entities:
            print("没有找到需要处理的实体节点")
            # 即使没有需要处理的节点，也尝试连接到既有索引
            try:
                vector_store = from_existing_index(self.embeddings, index_name)
                print("成功连接到现有向量索引")
                return vector_store
            except Exception:
                # 若索引不存在，尝试创建索引并连接（前提是库中已有 embedding）
                try:
                    ...
                    self.graph.query(
                        ...
                        FOR (e:`{effective_label}`) ON (e.`{embedding_property}`)
                        OPTIONS {{indexConfig: {{
    ...
    def _compute_embeddings_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            # 分析批处理的最佳大小
            embed_batch_size = min(10, len(embedding_tasks))
