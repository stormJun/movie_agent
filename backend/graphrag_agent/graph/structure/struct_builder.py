...
from graphrag_agent.graph.core import generate_hash
...
class GraphStructureBuilder:
    ...
    def __init__(self, batch_size=100, kb_prefix: str = ""):
        """
        初始化图结构构建器
        
        Args:
            batch_size: 批处理大小
            kb_prefix: 知识库命名空间前缀（用于同库隔离，比如 "movie"/"edu"；为空则不加前缀）
        """
        ...
        self.document_kb_label = kb_scoped_label_for_kb_prefix(self.kb_prefix, "document")
        self.chunk_kb_label = kb_scoped_label_for_kb_prefix(self.kb_prefix, "chunk")
        self.document_kb_label_fragment = (
            f":`{self.document_kb_label}`" if self.document_kb_label else ""
        )
        self.chunk_kb_label_fragment = (
            f":`{self.chunk_kb_label}`" if self.chunk_kb_label else ""
        )
    ...
    def with_prefix(self, value: str) -> str:
        """为任意字符串应用知识库前缀（如 'edu:' / 'movie:'）。"""
        return self._with_prefix(value)
            
    def clear_database(self):
        """
        清空数据库（可按命名空间前缀清理）。

        - 未设置 kb_prefix：沿用历史行为，清空全库（危险；同库共存场景请务必设置 kb_prefix）
        - 设置 kb_prefix：仅删除该前缀下的 Canonical/社区与源 Document 节点
        """
        if not self.id_prefix:
            clear_query = """
                MATCH (n)
        ...
        # 1) Canonical 层：__Entity__/__Chunk__/__Document__/__Community__
        self.graph.query(
            """
            MATCH (e:`__Entity__`)
        ...
        # 2) LangChain source 临时节点：Document{chunk_id: ...}
        self.graph.query(
            """
            MATCH (d:Document)
            WHERE d.chunk_id STARTS WITH $prefix
            DETACH DELETE d
            """,
            params={"prefix": prefix},
        )
    ...
    def create_document(self, type: str, uri: str, file_name: str, domain: str) -> Dict:
        ...
        file_name = self._with_prefix(file_name)
        query = f"""
        MERGE(d:`__Document__`{self.document_kb_label_fragment} {{fileName: $file_name}})
        SET d.type=$type, d.uri=$uri, d.domain=$domain, d.rawFileName=$raw_file_name
        RETURN d;
        """
    ...
    def create_relation_between_chunks(self, file_name: str, chunks: List) -> List[Dict]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            current_chunk_id = self._with_prefix(generate_hash(page_content))
            position = i + 1
    ...
    def _create_chunks_and_relationships_optimized(self, file_name: str, batch_data: List[Dict], 
        ...
        query_chunks_and_part_of = f"""
        UNWIND $batch_data AS data
        MERGE (c:`__Chunk__`{self.chunk_kb_label_fragment} {{id: data.id}})
        SET c.text = data.pg_content, 
            c.position = data.position, 
            c.length = data.length, 
            c.fileName = data.f_name,
            c.rawFileName = data.raw_file_name,
            c.content_offset = data.content_offset, 
            c.tokens = data.tokens
        WITH c, data
        MATCH (d:`__Document__`{self.document_kb_label_fragment} {{fileName: data.f_name}})
        MERGE (c)-[:PART_OF]->(d)
        """
        ...
        if first_relationships:
            ...
            MATCH (d:`__Document__`{self.document_kb_label_fragment} {{fileName: $f_name}})
            MATCH (c:`__Chunk__`{self.chunk_kb_label_fragment} {{id: relationship.chunk_id}})
        ...
        if next_relationships:
            query_next_chunk = f"""
            UNWIND $relationships AS relationship
            MATCH (c:`__Chunk__`{self.chunk_kb_label_fragment} {{id: relationship.current_chunk_id}})
            MATCH (pc:`__Chunk__`{self.chunk_kb_label_fragment} {{id: relationship.previous_chunk_id}})
    ...
    def parallel_process_chunks(self, file_name: str, chunks: List, max_workers=None) -> List[Dict]:
        ...
        if len(chunks) < 100:  # 对于小数据集，使用标准方法
            return self.create_relation_between_chunks(raw_file_name, chunks)
        ...
        def process_chunk_batch(batch, start_index):
            ...
            if start_index > 0 and start_index < len(chunks):
                ...
                current_chunk_id = self._with_prefix(generate_hash(prev_content))
                # 计算前面所有chunk的offset
