...
class DeepResearchTool(BaseSearchTool):
    ...
    def __init__(self, kb_prefix: str | None = None):
        ...
        # 初始化各种工具，用于不同阶段的搜索
        self.hybrid_tool = HybridSearchTool(kb_prefix=self.kb_prefix)  # 用于关键词提取和混合搜索
        self.global_tool = GlobalSearchTool(kb_prefix=self.kb_prefix)  # 用于社区检索
        self.local_tool = LocalSearchTool(kb_prefix=self.kb_prefix)    # 用于本地搜索
    ...
    @staticmethod
    def _dedupe_retrieval_payloads(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for item in items or []:
            ...
            if not source_id or not granularity:
                # 无法去重则保留为随机 key
                key = (source_id or str(id(item)), granularity or "unknown")
            else:
    ...
    @staticmethod
    def _build_reference_from_retrieval_payload(retrieval_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        chunks: set[str] = set()
    ...
    def retrieve_only(self, query_input: Any) -> Dict[str, Any]:
        """
        只做证据检索与聚合，不生成最终答案（不调用 deep/deeper 的推理/生成链）。

        注意：这里会调用底层工具的 retrieve_only()（local/global/hybrid）并合并证据。
        """
        overall_start = time.time()
        ...
        try:
            local_payload = self.local_tool.retrieve_only({"query": query})
            if isinstance(local_payload, dict):
                rr = local_payload.get("retrieval_results")
                if isinstance(rr, list):
                    retrieval_results.extend([x for x in rr if isinstance(x, dict)])
                ctx = local_payload.get("context")
                if isinstance(ctx, str) and ctx.strip():
                    context_parts.append(f"[local_search]\n{ctx.strip()}")
        except Exception:
            pass
        ...
        try:
            ...
            if isinstance(global_payload, dict):
                rr = global_payload.get("retrieval_results")
                if isinstance(rr, list):
                    retrieval_results.extend([x for x in rr if isinstance(x, dict)])
                ctx = global_payload.get("context")
                if isinstance(ctx, str) and ctx.strip():
                    context_parts.append(f"[global_search]\n{ctx.strip()}")
        except Exception:
            pass

        try:
            hybrid_payload = self.hybrid_tool.retrieve_only({"query": query})
            if isinstance(hybrid_payload, dict):
                rr = hybrid_payload.get("retrieval_results")
                if isinstance(rr, list):
                    retrieval_results.extend([x for x in rr if isinstance(x, dict)])

                low_level = hybrid_payload.get("low_level_content")
                if isinstance(low_level, str) and low_level.strip():
                    context_parts.append(f"[hybrid_search.low_level]\n{low_level.strip()}")

                high_level = hybrid_payload.get("high_level_content")
                if isinstance(high_level, str) and high_level.strip():
                    context_parts.append(f"[hybrid_search.high_level]\n{high_level.strip()}")
        except Exception:
            pass

        retrieval_results = self._dedupe_retrieval_payloads(retrieval_results)
