...
class DeeperResearchTool:
    ...
    def __init__(self, config=None, llm=None, embeddings=None, graph=None, kb_prefix: str | None = None):
        """
        初始化增强版深度研究工具
        ...
        self.kb_prefix = (kb_prefix or "").strip().rstrip(":")
        self.hybrid_tool = HybridSearchTool(kb_prefix=self.kb_prefix)
        ...
        # 5. 继承原有的深度研究工具功能
        self.deep_research = DeepResearchTool(kb_prefix=self.kb_prefix)
        ...
        # NOTE: v3 strict removes all caching (no disk cache, no cross-request memoization).
    ...
    def thinking(self, query: str) -> Dict[str, Any]:
        ...
        # 步骤1: 使用QueryGenerator生成初始子查询（不做缓存）
        ...
        # 步骤2: 社区感知和Chain of Exploration增强（不做缓存）
        ...
        for iteration in range(self.deep_research.max_iterations):
            ...
            if iteration > 0 and self.deep_research.all_retrieved_info:
                # 类似于 DeepSearch 中的 _generate_gap_queries：检查是否需要继续生成查询（不做缓存）
                gap_needed = (
                    len(self.query_generator.generate_followup_queries(query, self.deep_research.all_retrieved_info))
                    > 0
                )
...
""".strip()
        ...
        if exploration_path:
            ...
            for step in exploration_path:
                if step.get("step", 0) > 0:
                    if current_entity != step.get("node_id"):
                        current_entity = step.get("node_id")
                        path_summary.append(f"实体 {current_entity}: {step.get('reasoning', '')}")
            enhanced_prompt += "\n".join(path_summary[:3]) + "\n"
        ...
        # 添加矛盾分析结果
        if contradiction_result.get("contradictions"):
            enhanced_prompt += "\n信息矛盾分析：\n"
            for i, contradiction in enumerate(contradiction_result["contradictions"][:3]):
                if contradiction.get("type") == "numerical":
                    enhanced_prompt += (
                        f"- 矛盾{i+1}: 在数值上存在不一致，一处显示 {contradiction.get('value1')}，"
                        f"另一处显示 {contradiction.get('value2')}\n"
                    )
                else:
...
""".rstrip()
    ...
    def retrieve_only(self, query_input: Any) -> Dict[str, Any]:
        ...
        该工具本身具备更复杂的推理链路，但在 Phase 2 / retrieve_only 模式下，
        仅复用底层 DeepResearchTool 的 retrieve_only() 结果作为证据输出。
        """
        if isinstance(query_input, dict) and "query" in query_input:
            query = query_input["query"]
        else:
        ...
        payload = self.deep_research.retrieve_only({"query": query})
        if not isinstance(payload, dict):
            payload = {"query": query, "context": str(payload), "retrieval_results": [], "reference": {}}

        # 标记来源，便于调试/聚合
        payload = dict(payload)
        payload.setdefault("query", query)
        payload.setdefault("reference", {})
        payload["tool"] = "deeper_research"
