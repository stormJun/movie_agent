...
class GlobalSearchTool(BaseSearchTool):
    ...
    def __init__(self, level: int = None, kb_prefix: str | None = None):
        """
        初始化全局搜索工具
    ...
    def _get_community_data(self, keywords: List[str] = None) -> List[dict]:
        ...
        params = {"level": self.level, "community_prefix": self.community_id_prefix_filter}
    ...
    def retrieve_only(self, query_input: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            parts: list[str] = []
            for item in community_data or []:
                info = item.get("output", item) or {}
                cid = str(info.get("communityId") or info.get("id") or "").strip()
                text = (info.get("full_content") or info.get("summary") or "").strip()
                if not text:
                    continue
                header = f"Community: {cid}" if cid else "Community"
