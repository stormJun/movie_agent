...
class LocalSearchTool(BaseSearchTool):
    ...
    def __init__(self, kb_prefix: str | None = None):
        ...
        # 创建本地搜索器和检索器
        self.local_searcher = LocalSearch(self.llm, self.embeddings, kb_prefix=self.kb_prefix)
    ...
    def retrieve_only(self, query_input: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            context = "\n\n---\n\n".join([c.strip() for c in context_parts if c.strip()]).strip()
