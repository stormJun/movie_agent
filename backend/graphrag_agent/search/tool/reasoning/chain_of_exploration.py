...
def _cosine_similarity(a: Any, b: Any) -> float:
    ...
    Keep this module importable in "core-only" environments where heavy ML deps
    (scikit-learn/pandas) may be unavailable.
    """
    try:
        va = np.array(a, dtype=float).reshape(-1)
        vb = np.array(b, dtype=float).reshape(-1)
...
def _is_dataframe_like(value: Any) -> bool:
    # Avoid importing pandas; rely on duck typing.
    return (
        value is not None
...
class ChainOfExplorationSearcher:
    ...
    def __init__(self, graph, llm, embeddings_model, kb_prefix: str | None = None):
        """
        ...
        kb_slug = (kb_prefix or "").strip().rstrip(":")
        if kb_slug:
            prefix = f"{kb_slug}:"
            ...
            self.entity_id_prefix_filter = GRAPH_ENTITY_ID_PREFIX_FILTER or ""
            self.chunk_id_prefix_filter = GRAPH_CHUNK_ID_PREFIX_FILTER or ""
            self.community_id_prefix_filter = GRAPH_COMMUNITY_ID_PREFIX or ""
    ...
    def explore(self, query: str, starting_entities: List[str], max_steps: int = 5, exploration_width: int = 3):
        ...
        if self.entity_id_prefix_filter:
            starting_entities = [
                e for e in (starting_entities or []) if str(e).startswith(self.entity_id_prefix_filter)
            ]
    ...
    def _get_neighbors(self, entities):
        ...
        try:
            ...
            WHERE e.id IN $entity_ids
              ...
              AND ($entity_prefix = '' OR neighbor.id STARTS WITH $entity_prefix)
            ...
            # 转换为列表格式（避免 pandas 作为硬依赖）
            if _is_dataframe_like(result):
        ...
        for neighbor in neighbors:
            ...
            try:
                ...
                if description:
                    ...
                    similarity = _cosine_similarity(query_embedding, neighbor_embedding)
                else:
                    similarity = 0.0
    ...
    def _get_entity_info(self, entities):
        ...
        try:
            ...
            result = self.graph.query(
                query,
                params={"entity_ids": entities, "entity_prefix": self.entity_id_prefix_filter},
            )
            
            if not result or (hasattr(result, 'empty') and result.empty):
                return []
                
            # 转换结果（避免 pandas 作为硬依赖）
            if _is_dataframe_like(result):
        ...
        try:
            ...
            result = self.graph.query(
                ...
                params={
                    "entity_ids": entities,
                    "entity_prefix": self.entity_id_prefix_filter,
                    "chunk_prefix": self.chunk_id_prefix_filter,
                },
            )
    ...
    def _rank_content_by_relevance(self, query_embedding, content_list):
        ...
        for content in content_list:
            ...
            try:
                ...
                # 计算相似度
                similarity = _cosine_similarity(query_embedding, text_embedding)
