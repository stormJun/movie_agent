...
class CommunityAwareSearchEnhancer:
    ...
    def __init__(self, graph, embeddings_model, llm, kb_prefix: str | None = None):
        """
        初始化社区感知搜索增强器
    ...
    def find_relevant_communities(self, query: str, 
        ...
        try:
            communities = self.graph.query(
                community_query,
                params={"community_prefix": self.community_id_prefix_filter},
            )
    ...
    def extract_community_knowledge(self, communities: List[Dict]) -> Dict:
        ...
        try:
            ...
            WHERE ($chunk_prefix = '' OR chunk.id STARTS WITH $chunk_prefix)
            ...
            if entities:
                ...
                WITH e1, e2, r
                OPTIONAL MATCH (chunk1:__Chunk__)-[:MENTIONS]->(e1)
                WHERE ($chunk_prefix = '' OR chunk1.id STARTS WITH $chunk_prefix)
                WITH e1, e2, r, count(chunk1) as e1_mentions
                OPTIONAL MATCH (chunk2:__Chunk__)-[:MENTIONS]->(e2)
                WHERE ($chunk_prefix = '' OR chunk2.id STARTS WITH $chunk_prefix)
                WITH e1, e2, r, e1_mentions, count(chunk2) as e2_mentions
