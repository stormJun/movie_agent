...
class SimpleTextChunker:
    """简单分块器：按字符切分 + overlap，避免 HanLP 依赖导致的兼容性问题。"""
    ...
    def chunk_text(self, text: str) -> List[List[str]]:
        ...
        while start_pos < len(tokens):
            ...
            if end_pos >= len(tokens):
                break
            start_pos = max(0, end_pos - self.overlap)
...
class ChineseTextChunker:
    ...
    def __init__(self, chunk_size: int = CHUNK_SIZE, overlap: int = OVERLAP, max_text_length: int = MAX_TEXT_LENGTH):
        ...
        try:
            ...
            raise ImportError(
                "TEXT_CHUNKER_PROVIDER=hanlp 需要安装并可用的 hanlp 依赖；"
                "如果在本机不稳定，请改用 TEXT_CHUNKER_PROVIDER=simple"
            ) from e
