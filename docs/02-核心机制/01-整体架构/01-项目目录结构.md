...
| `docker/` | `docker/` | 部署编排（Neo4j/Postgres/mem0 等） |
...
├── logs/                      # 日志（运行产物；不提交）
├── .env.example               # 环境变量模板
├── docker/                    # Docker 编排（Neo4j/Postgres/mem0 等）
...
| **docker/docker-compose.yaml** | Docker 编排 | Neo4j、Postgres 等服务编排 |
...
| **DevOps** | `docker/`, `scripts/`, `.env` | 部署和运维 |
