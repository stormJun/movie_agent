                    ...
                    if use_stream:
                        ...
                        def handle_progress(progress: dict):
                            try:
                                ...
                                agent_type = str(progress.get("agent_type", "")).strip()
                                retrieval_count = progress.get("retrieval_count")
                                label = "检索中" if stage == "retrieval" else "处理中"
                                if agent_type:
                                    label += f" ({agent_type})"
                                if isinstance(retrieval_count, int):
                                    label += f"，命中 {retrieval_count}"
                                progress_placeholder.caption(label)
                            except Exception as e:
                                print(f"处理进度出错: {str(e)}")
                        
                        # 使用流式 API
                        with st.spinner("思考中..."):
                            try:
                                raw_thinking = send_message_stream(
                                    prompt, handle_token, on_progress=handle_progress
                                )
                                # 检查是否有响应
