...
// 禁用缩进代码块功能，避免四个连续空格被识别为代码块
// 这样可以保留围栏代码块（```）功能，只禁用缩进代码块
...
function parse(md, options) {
    ...
    // 判断tokens里面是否存在markdown
    if (Array.isArray(tokens) && tokens.length > 0) {
        const markdown = tokens[0];
        if (markdown.type === 'fence' && markdown.params === 'markdown') {
            tokens = parser.parse(markdown.content, {});
        }
        // for (const index in tokens) {
        //     const token = tokens[index];
        //     if (token.type === 'fence') {
        //         console.log('token.content:', token.content);
        //         // tokens = parser.parse(token.content, {});
        //     }
        // }
    }
