...
```
graph-rag-agent/
├── backend/graphrag_agent/         # 🎯 核心包 - GraphRAG智能体系统
...
│   ├── config/             # ⚙️ 配置模块
│   │   ├── prompts/        # 提示模板集合
│   │   └── settings.py     # 核心默认配置与类型（业务语义已迁至 backend/config/rag_semantics.py）
│   ├── evaluation/         # 📊 评估系统
│   │   ├── core/           # 评估核心组件
│   │   ├── metrics/        # 评估指标实现
│   │   └── test/           # 评估测试脚本
...
├── backend/infrastructure/         # 🔧 基础设施层（集成/管道/模型/路由/DB）
│   ├── agents/             # Agent 工厂与生命周期
│   ├── config/             # ⚙️ 基础设施配置
│   │   ├── graphrag_settings.py # core overrides 注入入口（把语义/环境注入到 core settings）
│   │   ├── neo4jdb.py      # 数据库连接管理
│   │   ├── semantics.py    # 语义桥接（infra 读取语义的唯一入口）
│   │   └── settings.py     # 基础设施默认值
│   ├── integrations/       # 🔌 集成模块
│   │   └── build/          # 🏗️ 知识图谱构建
│   ├── models/             # 🧩 模型管理
...
│   ├── streaming/          # 📡 流式输出
│   └── utils/              # 🧰 工具函数
├── backend/config/                 # ⚙️ 服务配置（settings/database/rag）
│   ├── rag_semantics.py            # RAG 语义配置（schema/示例/回答格式等）
│   ├── rag.py                      # 对外“server-facing”语义导出（避免业务侧直接依赖 core）
├── backend/server/                 # 🖧 后端服务（API入口）
│   ├── main.py             # FastAPI应用入口
│   ├── api/                # 新版 REST API
│   └── models/             # 数据模型
