...
class IndexCommunityBuilder:
    ...
    def build_index_and_communities(self):
        ...
        try:
            # 1. 创建实体索引
            run_entity_index = self._get_env_bool("BUILD_RUN_ENTITY_INDEX", True)
            if run_entity_index:
                ...
                self.console.print("[yellow]跳过实体索引创建（BUILD_RUN_ENTITY_INDEX=false）[/yellow]")
                self.performance_stats["索引创建"] = 0
            
            # 2. 检测和合并相似实体
            similar_start = time.time()
            duplicates = []
            if GRAPH_SKIP_SIMILAR_ENTITY:
                self.console.print("[yellow]跳过相似实体检测（GRAPH_SKIP_SIMILAR_ENTITY=true）[/yellow]")
            else:
                self.console.print("[cyan]正在检测相似实体...[/cyan]")
                duplicates, _stats = self.entity_detector.process_entities()
            ...
            merged_count = 0
            if GRAPH_SKIP_ENTITY_MERGE:
                self.console.print("[yellow]跳过实体合并（GRAPH_SKIP_ENTITY_MERGE=true）[/yellow]")
            elif not duplicates:
                self.console.print("[yellow]未发现候选重复实体，跳过合并[/yellow]")
            else:
            ...
            quality_result = {'disambiguated': 0, 'aligned': 0}
            if GRAPH_SKIP_ENTITY_QUALITY:
                self.console.print("[yellow]跳过实体质量提升（GRAPH_SKIP_ENTITY_QUALITY=true）[/yellow]")
            else:
            ...
            # 5. 社区检测
            run_community_detection = self._get_env_bool("BUILD_RUN_COMMUNITY_DETECTION", True)
            community_start = time.time()
            community_results = None
            if GRAPH_SKIP_COMMUNITY:
                self.console.print("[yellow]跳过社区检测（GRAPH_SKIP_COMMUNITY=true）[/yellow]")
            elif not run_community_detection:
                self.console.print("[yellow]跳过社区检测（BUILD_RUN_COMMUNITY_DETECTION=false）[/yellow]")
            else:
            ...
            summaries = []
            if GRAPH_SKIP_COMMUNITY_SUMMARY:
                self.console.print("[yellow]跳过社区摘要（GRAPH_SKIP_COMMUNITY_SUMMARY=true）[/yellow]")
            else:
