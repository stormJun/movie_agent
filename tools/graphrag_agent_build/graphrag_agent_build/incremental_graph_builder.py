...
from graphrag_agent_build.incremental.file_change_manager import FileChangeManager
...
class IncrementalGraphUpdater:
    ...
    def __init__(self, files_dir: str, registry_path: str = None):
        ...
        self.kb_prefix = DOCUMENT_KB_PREFIX
        ...
        self.struct_builder = GraphStructureBuilder(
            batch_size=BATCH_SIZE, kb_prefix=self.kb_prefix
        )
    ...
    def update_changed_file_embeddings(self, changed_files: List[str]) -> Dict[str, int]:
        ...
        marked_chunks = self.embedding_manager.mark_changed_files_chunks(
            changed_files, kb_prefix=self.kb_prefix
        )
        
        # 查找这些Chunk关联的实体，标记需要更新Embedding
        query = """
        MATCH (c:`__Chunk__`)-[:MENTIONS]->(e:`__Entity__`)
        WHERE (coalesce(c.rawFileName, c.fileName) IN $filenames OR c.needs_reembedding = true)
          AND ($prefix = '' OR c.id STARTS WITH $prefix)
        ...
        result = self.graph.query(
            ...
            params={
                "filenames": filenames,
                "prefix": f"{self.kb_prefix}:" if self.kb_prefix else "",
            },
        )
    ...
    def process_deleted_files(self, deleted_files: List[str]) -> int:
        ...
        for file_path in deleted_files:
            ...
            file_name = self.struct_builder.with_prefix(raw_file_name)
            ...
            if not chunk_result or not chunk_result[0]["chunk_ids"]:
                self.console.print(f"[yellow]文件 {raw_file_name} 没有相关的数据需要删除[/yellow]")
                continue
            ...
            self.console.print(f"[blue]已删除文件 {raw_file_name} 相关数据: "
