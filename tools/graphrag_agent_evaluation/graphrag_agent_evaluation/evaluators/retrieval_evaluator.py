...
from graphrag_agent_evaluation.core.base_evaluator import BaseEvaluator
from graphrag_agent_evaluation.core.evaluation_data import (
    RetrievalEvaluationData,
    RetrievalEvaluationSample,
)
from graphrag_agent_evaluation.preprocessing.reference_extractor import (
...
class GraphRAGRetrievalEvaluator(BaseEvaluator):
    ...
    def _prepare_entity_relation_maps(self):
        ...
        try:
            ...
            LIMIT 2000
            """
            entity_result = self.neo4j_client.query(entity_query)
            ...
            LIMIT 1000
            """
            relation_result = self.neo4j_client.query(relation_query)
    ...
    def _get_relevant_graph_data(self, question: str) -> Tuple[List[str], List[Tuple[str, str, str]]]:
        ...
        try:
            ...
            if entities:
                ...
                rel_result = self.neo4j_client.query(rel_query, {"entity_ids": entities})
            ...
            if len(entities) < 3:
                ...
                chunk_result = self.neo4j_client.query(chunk_query, {"keywords": question_words})
                ...
                if chunk_ids:
                    ...
                    chunk_entity_result = self.neo4j_client.query(
                        chunk_entity_query, {"chunk_ids": chunk_ids}
                    )
                    
                    if chunk_entity_result:
                        for record in chunk_entity_result:
