...
from graphrag_agent_evaluation.core.base_metric import BaseMetric
from graphrag_agent_evaluation.utils.text_utils import normalize_answer
            ...
            if agent_type == "naive":
                ...
                if self.neo4j_client and chunks:
                    try:
                        ...
                        """
                        result = self.neo4j_client.query(query, {"chunk_ids": chunks})
                        
                        entity_ids = []
                        if result and result[0].get("entity_ids"):
                            entity_ids = result[0].get("entity_ids")
                        ...
                        if entity_ids:
                            ...
                            LIMIT 3
                            """
                            community_result = self.neo4j_client.query(
            ...
            if self.neo4j_client:
                try:
                    ...
                    if entity_ids:
                        ...
                        LIMIT 5
                        """
                        result = self.neo4j_client.query(community_query)
                    ...
                    if not community_info:
                        ...
                        LIMIT 10
                        """
                        all_result = self.neo4j_client.query(all_query)
...
class SubgraphQualityMetric(BaseMetric):
    ...
    def _get_processed_relationships(self, relationships) -> List[Tuple[str, str, str]]:
        ...
        if string_id_rels and self.neo4j_client:
            try:
                ...
                LIMIT 1000
                """
                result = self.neo4j_client.query(query)
                ...
                if not processed_relationships:
                    ...
                    LIMIT 10
                    """
                    mentions_result = self.neo4j_client.query(mentions_query)
                ...
                if not processed_relationships and len(string_id_rels) >= 2:
                    ...
                    entity_result = self.neo4j_client.query(entity_query, {"ids": entity_ids})
    ...
    def _get_relationships_from_ids(self, rel_ids: List[str]) -> List[Tuple[str, str, str]]:
        ...
        try:
            ...
            """
            result = self.neo4j_client.query(query, {"ids": numeric_rel_ids})
            ...
            if not relationships:
                ...
                RETURN a.id AS source, type(r) AS relation, b.id AS target
                """
                alt_result = self.neo4j_client.query(alt_query, {"ids": numeric_rel_ids})
    ...
    def _evaluate_naive_coverage(self, sample, keywords: List[str]) -> float:
        ...
        if chunks and self.neo4j_client and keywords:
            try:
                ...
                """
                result = self.neo4j_client.query(query, {"ids": chunks})
    ...
    def _get_graph_data(self, entity_ids: List[str], rel_ids: List[str]) -> Tuple[Dict[str, str], List[Dict]]:
        ...
        if entity_ids:
            try:
                ...
                """
                result = self.neo4j_client.query(query, {"ids": entity_ids})
        ...
        if rel_ids:
            try:
                ...
                if numeric_ids:
                    ...
                    """
                    result = self.neo4j_client.query(query, {"ids": numeric_ids})
    ...
    def _calculate_connectedness_score(self, entity_ids: List[str], relationship_info: List[Dict]) -> float:
        ...
        if connected_ratio < 0.1 and self.neo4j_client and len(entity_ids) >= 2:
            try:
                ...
                path_count = 0
                if result and result[0].get("path_count") is not None:
                    path_count = result[0].get("path_count")
...
class EntityCoverageMetric(BaseMetric):
                    ...
                    for entity_id in entity_ids:
                        ...
                        try:
                            if self.neo4j_client:
                                ...
                                LIMIT 10
                                """
                                result = self.neo4j_client.query(query, {"id": entity_id})
    ...
    def _calculate_graph_relevance(self, entity_ids: List[str], keywords: List[str]) -> float:
        ...
        try:
            ...
            rel_count = 0
            if result and result[0].get("rel_count") is not None:
                rel_count = result[0].get("rel_count")
